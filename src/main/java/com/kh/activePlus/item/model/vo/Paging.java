...
public class Paging {
	public String pageIndexList(int totalCnt, int current_page) {
		...
		// 만약, 총 페이지보다 현재 페이지가 높다면
		// 현재 페이지를 강제로 총페이지로 변경한다
		if(totalPage < page) {
			page = totalPage;
		}
		// 시작페이지
	...
	public String pageIndexLista (int totalCnt, int current_page, String i_div_seq) {
		...
		if (startPage > 1) {
			result = "<li><a href=\"?"+i_div_seq+"&page=1\">처음</a></li>";
		}

		if (page > 10) {
			result += "<li><a href=\"?"+i_div_seq+"&page=" + (startPage - 1)  + "\">이전</a></li>";
		}

		for (int iCount = startPage; iCount <= endPage; iCount++) {
			if (iCount == page) {
				result += "<li class='active'><a href='?"+i_div_seq+"&page=" + iCount  + "'>" + iCount + "</a></li>";
			} else {
				result += "<li><a href='?"+i_div_seq+"&page=" + iCount  + "'>" + iCount + "</a></li>";
			}
		}

		if ((startPage + 10) < totalPage) {
			result += "<li><a href=\"?"+i_div_seq+"&page=" + (startPage + 10)  + "\">다음</a></li>";
		}


		if (endPage < totalPage) {
			result += "<li><a href=\"?"+i_div_seq+"&page=" + totalPage + "\">끝</a></li>";
		}
